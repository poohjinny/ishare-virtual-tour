...
import {
  ...
  tourBreadcrumbNearbyPlacesTooltip,
...
interface TourBreadcrumbSiblingMenuProps {
  ...
  crumbId: string;
  label: string;
  /** Full scene title — tooltips when the trail label is shortened. */
  title: string;
...
export function TourBreadcrumbSiblingMenu({
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-current={
                    highlighted && isCurrentCrumb ? 'true' : undefined
                  }
                  className={cn(
                    tourNavBreadcrumbSiblingOptionClassName,
                    highlighted && !isCurrentCrumb && 'font-semibold',
                  )}
                  disabled={!open}
  ...
  // Split control: the title goes to that place, the chevron opens the siblings.
  return (
    <span
      ref={rootRef}
      className={cn(tourNavBreadcrumbSplitClassName, 'gap-px')}
    >
      {isCurrentCrumb ?
        <IconTooltip
          label={label !== title ? title : TOUR_BREADCRUMB_CURRENT_TOOLTIP}
          placement='bottom'
      ...
      : <IconTooltip
          label={tourBreadcrumbGoToTooltip(title)}
          placement='bottom'
          className='max-w-full'
          disabled={disabled}
        >
