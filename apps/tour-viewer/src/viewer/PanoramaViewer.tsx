...
import type { AnchoredShareMenuPayload } from '../components/AnchoredShareMenu';
...
interface PanoramaViewerProps {
  ...
  /**
   * Desktop share from in-scene panels — open the anchored mini Share menu
   * (mobile still uses native share via {@link shareTourView}).
   */
...
export const PanoramaViewer = forwardRef<TourViewerHandle, PanoramaViewerProps>(
  function PanoramaViewer(
    ...
    const onOpenAnchoredShareMenuRef = useLatestRef(onOpenAnchoredShareMenu);
    ...
    > | null>(null);
    const reportViewerLoadErrorRef = useRef<
      (info: ViewerLoadErrorInfo) => void
    >(() => {});
      ...
      applyTourUpdate: async (nextTour, options) => {
        ...
        for (const scene of Object.values(nextTour.scenes)) {
          ...
          if (patch.panorama && patch.id === currentId) {
            // updateNode() on the current node with a changed panorama internally
            // triggers setCurrentNode({ forceUpdate }), which clears then re-adds
            // the node markers. We must NOT fire a second reload ourselves — the
            // two concurrent reloads race and the late clearMarkers() drops the
            // hotspots. Pass the full node (incl. markers) and wait for the
            // internal reload's node-changed before revealing hotspots.
            const fullNode =
              nodeConfigs.find((node) => node.id === currentId) ?? patch;
            reloadPromise = new Promise<void>((resolve) => {
              let done = false;
              const settle = () => {
                if (done) return;
                done = true;
                virtualTour.removeEventListener('node-changed', onNodeChanged);
                window.clearTimeout(timer);
                resolve();
              };
              const onNodeChanged = () => settle();
              const timer = window.setTimeout(
                settle,
                PANORAMA_RELOAD_TIMEOUT_MS,
              );
              virtualTour.addEventListener('node-changed', onNodeChanged);
            });
            virtualTour.updateNode(fullNode);
    ...
    useEffect(() => {
          ...
          [
            VirtualTourPlugin,
            {
              ...
              transitionOptions: (_node: unknown, fromNode?: unknown) => {
                const effect = resolveTourSceneTransitionEffect(
                  tourRef.current,
                );
                return fromNode ?
      ...
      const handlePanelPointerDown = (event: PointerEvent) => {
        ...
        if (navPanel) {
          ...
          if (shareButton instanceof HTMLButtonElement) {
            ...
            const contextLabel =
              openGraph.contextLabel?.trim() ||
              tour.scenes[navTarget.sceneId]?.title?.trim() ||
              openGraph.title;
            ...
            onOpenAnchoredShareMenuRef.current?.({
              shareUrl,
              message,
              contextLabel,
      ...
      const notifyFirstPanoramaInteract = () => {
        if (!trackFirstPanoramaInteract || firstPanoramaInteractNotified)
          return;
