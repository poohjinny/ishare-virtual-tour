  ...
  function ThreeDViewer(
      ...
      (hotspots: Hotspot[], scene3d: THREE.Scene) => {
        ...
        for (const hs of hotspots) {
          ...
          if (hs.type === 'nav') {
            ...
            const infoAriaLabel =
              naming ?
                buildNamingHotspotAriaLabel(hs, tourRef.current)
              : (hs.popup?.title?.trim() ?? hs.label?.trim() ?? ariaLabel);
            wrap.innerHTML = `
              <button type="button" class="hotspot-info${statusSold}"
                ...
                <span class="hotspot-info__pill">
                  ...
                  <span class="hotspot-info__label">${buildNamingHotspotPillLabelHtml(hs, tourRef.current)}</span>
                </span>
              </button>`;
          }
    ...
    useImperativeHandle(ref, () => ({
      ...
      focusHotspot: (hotspotId, options) => {
        ...
        if (!found) return;
        const targetView =
          found.hotspot.targetView ??
          tour.scenes[found.hotspot.sceneId ?? found.sceneId ?? '']
            ?.defaultView;
        const camera = cameraRef.current;
        const controls = controlsRef.current;
        if (targetView && camera && controls) {
          void waitForSceneTransition(
            camera,
            controls,
            targetView,
            modelRootRef.current,
            HOTSPOT_CAMERA_TRANSITION_TIMING,
          ).then(() => openNamingHotspotById(hotspotId));
          return;
        }
        openNamingHotspotById(hotspotId);
      },
